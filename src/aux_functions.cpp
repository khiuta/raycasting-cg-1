    ...
    else if (line[0] == 'f')
    {
      ...
      for (size_t i = 2; i < line.size(); i++)
      {
        if (line[i] == ' ')
        {
          ...
          vertex_control = 0;
          if (point_control > 3)
            break; // Proteção: ignora vértices extras em polígonos > 4 lados
        }
        else if (line[i] == '/')
          vertex_control++;
        else if (line[i] >= 48 && line[i] <= 57)
        {
          ...
          if (vertex_control == 0)
            vertex_indices[point_control] = val;
          else if (vertex_control == 1)
            tex_vertex_indices[point_control] = val;
          else if (vertex_control == 2)
            nor_vertex_indices[point_control] = val;
          i = k - 1;
        }
      }
      ...
      if (!check_idx(vertex_indices[0], v.size()) ||
          ...
          !check_idx(vertex_indices[2], v.size()))
        continue;
