...
#include "../utils/Rectangle.hpp" // [NOVO] Incluindo o Espelho
...
Point4 lookFrom(15.0f, 5.0f, 55.0f);
Point4 lookAt(8.f, 5.0f, 0.0f);
...
// [MANTIDO] Sua função original de cor local
...
// [NOVO] Helper para refletir raio (mesma lógica usada no Phong, mas isolada)
Vector4 reflect_ray(const Vector4& v, const Vector4& n) {
    return v - n * 2.0f * dot(v, n);
}

// [NOVO] Função Recursiva de Ray Tracing
Point3 cast_ray(const Point4& ray_origin, const Vector4& ray_dir, int depth) {
    // Fundo estrelado se acabar a recursão
    ...
    for(const auto& object : world){
      HitRecord temp_rec;
      // t_min levemente maior que 0 para evitar acne
    ...
    if(hit_anything){
        // 1. Calcula cor local (texturas, sombras, luzes)
        ...
        // Se não bateu em nada, retorna o céu estrelado
...
void raycast(std::ofstream &image, int lin_start, int col_start, int width, int height) {
  ...
  for(int l = lin_start; l < height; l++){
    for(int c = col_start; c < width; c++){
      ...
      // [ALTERADO] Substitui a lógica de loop de interseção direta pela chamada recursiva
      Point3 final_color = cast_ray(ray_origin, ray_dir, 3); // Profundidade 3
...
void read_obj_file(const std::string& filename,
                   ...
                   AABB &aabb,
                   ListMesh *mesh){
    // ... (MANTIDO IGUAL AO SEU CÓDIGO ORIGINAL - omitido para brevidade, mantenha a implementação aqui) ...
    // Se precisar, copie e cole sua função read_obj_file aqui, ela não precisa mudar.
...
int main() {
  ...
  // ... (strips mantidas, omitido para brevidade) ...
  ...
  for(auto& face : road_strip1->faces){ face->color = Point3(1,1,1); face->dif_color = Point3(1,1,1); }
  ...
  for(auto& face : road_strip2->faces){ face->color = Point3(1,1,1); face->dif_color = Point3(1,1,1); }
  ...
  for(auto& face : road_strip3->faces){ face->color = Point3(1,1,1); face->dif_color = Point3(1,1,1); }
  ...
  for(auto& face : road_strip4->faces){ face->color = Point3(1,1,1); face->dif_color = Point3(1,1,1); }
  ...
  for(auto& face : road_strip5->faces){ face->color = Point3(1,1,1); face->dif_color = Point3(1,1,1); }
  ...
  for(auto& face : road_strip6->faces){ face->color = Point3(1,1,1); face->dif_color = Point3(1,1,1); }
  #pragma endregion

  auto post_base = std::make_unique<Cylinder>(Point4(-15.0f, 0.0f, 5.0f), 15.0f, 1.0f, Vector4(0.0f, 1.0f, 0.0f), true, true, post_color.color, post_color.color, post_color.spec);
  auto post_arm = std::make_unique<Cylinder>(Point4(-15.0f, 16.0f, 4.0f), 6.0f, 1.0f, Vector4(0.0f, 0.0f, 1.0f), true, true, post_color.color, post_color.color, post_color.spec);
  auto lamp = std::make_unique<Sphere>(Point4(-15.0f, 16.0f, 11.0f), 1.0f, lamp_color.color, lamp_color.color, lamp_color.spec);

  auto post_base2 = std::make_unique<Cylinder>(Point4(20.0f, 0.0f, 5.0f), 15.0f, 1.0f, Vector4(0.0f, 1.0f, 0.0f), true, true, post_color.color, post_color.color, post_color.spec);
  auto post_arm2 = std::make_unique<Cylinder>(Point4(20.0f, 16.0f, 5.0f), 6.0f, 1.0f, Vector4(0.0f, 0.0f, 1.0f), true, true, post_color.spec, post_color.spec, post_color.spec);
  auto lamp2 = std::make_unique<Sphere>(Point4(20.0f, 16.0f, 11.0f), 1.0f, lamp_color.color, lamp_color.color, lamp_color.spec);
  
  #pragma region road cones
  auto road_cone1 = std::make_unique<Cone>(Point4(0.0f, 0.0f, 10.0f), 5.0f, true, Point4(0.0f, 2.0f, 10.0f), road_cone_color.color, road_cone_color.color, road_cone_color.spec);
  ...
  for(auto& face : road_cone_base1->faces){ face->color = road_cone_color.color; face->dif_color = road_cone_color.color; face->spec_color = road_cone_color.spec; }

  auto road_cone2 = std::make_unique<Cone>(Point4(5.0f, 0.0f, 10.0f), 5.0f, true, Point4(5.0f, 2.0f, 10.0f), road_cone_color.color, road_cone_color.color, road_cone_color.spec);
  ...
  for(auto& face : road_cone_base2->faces){ face->color = road_cone_color.color; face->dif_color = road_cone_color.color; face->spec_color = road_cone_color.spec; }

  auto road_cone3 = std::make_unique<Cone>(Point4(-5.0f, 0.0f, 10.0f), 5.0f, true, Point4(-5.0f, 2.0f, 10.0f), road_cone_color.color, road_cone_color.color, road_cone_color.spec);
  ...
  for(auto& face : road_cone_base3->faces){ face->color = road_cone_color.color; face->dif_color = road_cone_color.color; face->spec_color = road_cone_color.spec; }
  #pragma endregion
...
world.push_back(std::make_unique<Plain>(Point4(0, 0, 0), Vector4(0, 1, 0), floor_col, floor_col, specular_plains));
  ...
  // [MODIFICADO] Espelho Mais Perto e Mais Longo
 Point4 mirror_origin(-14, 1, -10); 
  
  // Ponto que define a LARGURA (X positivo, Y=0, Z fixo) -> Comprimento de 160
  Point4 mirror_width_pt(17, 1, -10);   
  
  // Ponto que define a ALTURA (X fixo, Y alto, Z fixo) -> Altura de 60
  Point4 mirror_height_pt(-14, 9.2f, -10); 
  
  Point3 mirror_color(0, 0, 0); 
  Point3 mirror_spec(1, 1, 1);
  
  auto espelho = std::make_unique<Rectangle>(
      mirror_origin,     // P0
      mirror_width_pt,   // P1 (define eixo horizontal)
      mirror_height_pt,  // P2 (define eixo vertical)
      mirror_color, mirror_color, mirror_spec, 
      .5f // 100% reflexivo para facilitar o debug
  );
  world.push_back(std::move(espelho));
